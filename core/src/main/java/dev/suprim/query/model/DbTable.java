...
public record DbTable(
	...
	public DbTable copyWithAlias(String tableAlias) {
		List<DbColumn> columns = dbColumns.stream()
		                                  .map(col -> col.copyWithTableAlias(
				                                  tableAlias))
		                                  .toList();
		return new DbTable(
				schema,
	...
	private String parseChainedArrows(String expression) {
		...
		while (i < expression.length()) {
			if (i + 2 < expression.length() && expression.startsWith(
					"->>",
					i
			)) {
				...
				i += key.length();
				result.append("->>").append(isQuoted(key) ? key :
						"'" + key + "'");
	...
	private boolean isQuoted(String key) {
		return key.length() >= 2 && key.charAt(0) == '\'' && key.charAt(
				key.length() - 1) == '\'';
	}
