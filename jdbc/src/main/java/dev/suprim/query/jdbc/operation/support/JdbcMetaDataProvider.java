package dev.suprim.query.jdbc.operation.support;
...
public class JdbcMetaDataProvider implements DatabaseMetaDataCallback<DbMeta> {
    ...
    private final List<MetaDataExtraction> exclusions = new ArrayList<>();

    public void addExtraction(MetaDataExtraction extraction) {
        exclusions.add(extraction);
    }

    @Override
    public DbMeta processMetaData(@NonNull DatabaseMetaData databaseMetaData) throws SQLException {
        ...
        List<DbTable> dbTables = metaDataExclusion.get().getTables(
                databaseMetaData,
                includeAllSchemas,
                includedSchemas
        );
    ...
    private Optional<MetaDataExtraction> getMetaDataExtraction(String productName) {
        return this.exclusions.stream()
                .filter(metaDataExtraction -> metaDataExtraction.canHandle(productName))
                .findFirst();
    }
}
