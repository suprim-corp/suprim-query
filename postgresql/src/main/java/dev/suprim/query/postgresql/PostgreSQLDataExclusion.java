...
public class PostgreSQLDataExclusion implements MetaDataExtraction {
    private static final List<String> EXCLUDED_SCHEMAS = Arrays.asList(
            "pg_catalog",
            "information_schema"
    );
    ...
    public List<DbTable> getTables(
        ...
        try {
            ...
            if (includeAllSchemas) {
                includedSchemas = getAllSchemas(databaseMetaData, EXCLUDED_SCHEMAS);
            }
            ...
            for (String schema : includedSchemas) {
                ...
                List<DbTable> tables = metaDataTables.stream()
    ...
    private DbTable getDbTable(DatabaseMetaData databaseMetaData, MetaDataTable metaDataTable) {
        try {
            List<DbColumn> columns = getAllColumns(
                    databaseMetaData,
            ...
            String schema = metaDataTable.schema();
            String schemaName = (nonNull(schema) && !schema.isBlank())
                    ? schema
                    : metaDataTable.catalog();
